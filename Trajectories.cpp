 ...
 *
 *  Description:
 *  This code performs numerical calculations to obtain trajectories
 *  in ergodic billiards with rotational symmetries in their boundary
 *
...
double R,r,PI,sqrt3;
int i,j,n,sectors,lad, pointsR,pointsline,pointsr,pointstotal,ladaux,rest,divi,sectorcol,sectoraux,cont,ok,sectorprevious,cont_traj;
...
double y11, y22,deriy,angy, vxfinal,vyfinal,xmin,xmax,ymin,ymax;
...
int main(){
  ...
  cont_traj = 100000;	//Maximum number of collisions
...
// The choice of values for a and b will depend on the chosen symmetry n
  ...
  //Points between sectors' border Pontos entre os sectors da borda
  i = 4;
  while(i<(sectors+1)) {
  ...
  //Border discretizations of sectors 1, 2, and 3
  ...
  //Radius 'R' circunferecences centers
  i=1;
  while(i<n){
  ...
  while(cont<(cont_traj+1)) {
    ...
    while(i<(n*pointstotal +1)) {
      ...
      if (( (xmin < xtest ) && (xmax > xtest )  )  && ( (ymin < ytest ) && (ymax > ytest )   ) ) {
        ...
        j=0; //Interaction with radius R circunference sectors. Sectors 1, 4, 7, 10, ...
        while(j<n) {
    ...
    j=0; //Interaction with r radius circunference sectors. Sectors 3, 6, 9, 12, ...
    while(j<n){
   ...
   sectorprevious = sectoraux;	
		
   atraj = (vyfinal/vxfinal);
   btraj = yfinal - atraj*xfinal;	
	
   fprintf(output,"%d %d %.12f %.12f %.12f %.12f\n",cont,ladaux,xfinal,yfinal,vxfinal,vyfinal); // xfinal and yfinal are the particle’s positions at the moments of each collision with the boundary
