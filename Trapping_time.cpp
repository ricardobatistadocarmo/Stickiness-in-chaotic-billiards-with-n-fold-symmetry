 ...
 *
 *  Description:
 *  This code numerically obtains the different trapping times that occur in the dynamics 
 *  for a given boundary, defined by the symmetry parameter and the radii of the circunferences.
 *
...
double R,r,PI,sqrt3;
int i,j,n,sectors,lad, pointsR,pointsline,pointsr,pontostotal,ladaux,rest,divi,sectorcol,sectoraux,cont,ok,sectorprev,ladmax,k;
...
int box_matrix[1001][1001],ifinal,jfinal,ICs,IC,IC_cont,temptraj,conttrajfinal,contfinal,contstick,count,calc ;

double *xdisc, *ydisc, *coefang, *coelinear, *ang, *sum_mr ;
...
int main(){
  ...
  temptraj = 20000000; //Maximum number of collisions
  ICs = 1; // Number of initial conditions
  ...
  FILE* output;
  output = fopen("c3_Data.dat","w");	

  FILE* output2;
  output2 = fopen("c3_traj.dat","w");	
  ...
  while(i<(  n*pontostotal + 1 )  ){
	...
	if (( (xmin < xtest ) && (xmax > xtest )  )  && ( (ymin < ytest ) && (ymax > ytest )   ) ){
	  ...
	  j = 0;  //Interaction between radius 'R' circunferences sectors. Sectors 1, 4, 7, 19, ...
	  while(j<n){
	  ...
	  j = 0;  //Interaction with line sectors 2, 5, 8, 11, ...
	  while(j<n){
      ...
      while(i<(  n*pontostotal + 1 )  ){
        ...
        sum += sqrt(  ( xdisc[i+1] - xdisc[i]  )*( xdisc[i+1] - xdisc[i]  ) + ( ydisc[i+1] - ydisc[i] )*( ydisc[i+1] - ydisc[i] )  );
        i++;
